 ...
 * Acts as a container for chat and other right-side panels
 * Supports resizable width with persistent storage
 */
@customElement('my-right-panel')
export class RightPanel extends LitElement {
  ...
  private readonly DEFAULT_WIDTH = 420
  ...
  private handleResizeStart(e: MouseEvent) {
    ...
    if (this.panelContainer && !this.resizeHandler) {
      this.resizeHandler = new ResizeHandler(this.panelContainer, {
        ...
        invertDirection: true, // Use inverted direction for right panel
        onResize: (width) => {
      ...
      <div class="panel-container" style=${styleMap({
        ...
        <div class="tool-window-header no-select">
          ...
          <div class="header-actions">
            <button class="close-button" @click=${() => toggleChatPanel()} title="Close panel">
  ...
  static styles = [
    ...
    css`
      ...
      /* Override the global resize handle styles for left position */
      .resize-handle.left {
        left: -3px;
        right: auto;
        width: 4px;
