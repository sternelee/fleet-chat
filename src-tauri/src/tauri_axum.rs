...
mod tests {
    ...
    mod local_response_tests {
        ...
        async fn test_response_creation_with_body() {
            let response = Builder::new().status(200).body(Body::from("test body")).unwrap();
        ...
        async fn test_internal_server_error() {
            ...
            assert_eq!(response.status_code, 500);
            assert!(String::from_utf8(response.body).unwrap().contains(error_message));
        ...
        fn test_sse_response_creation() {
            ...
            assert_eq!(sse_response.headers.get("cache-control").unwrap(), "no-cache");
            assert_eq!(sse_response.headers.get("connection").unwrap(), "keep-alive");
        ...
        async fn test_sse_response_from_axum_response() {
            ...
            assert_eq!(local_response.headers.get("content-type").unwrap(), "text/event-stream");
        }
