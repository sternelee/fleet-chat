...
function convertSourceFile(sourcePath, targetPath, pluginName) {
  ...
  content = content.replace(
    /from ['"]@raycast\/utils['"]/g,
    "from '@fleet-chat/raycast-api/utils'"
  );
...
function convertRaycastPlugin(raycastPath, options = {}) {
  ...
  if (existsSync(sourceDir)) {
    ...
    const sourceFiles = readdirSync(sourceDir).filter(f => f.endsWith('.ts') || f.endsWith('.tsx') || f.endsWith('.jsx'));
    ...
    subdirs.forEach(dir => {
      ...
      const files = readdirSync(subdirPath).filter(f => f.endsWith('.ts') || f.endsWith('.tsx') || f.endsWith('.jsx'));
      files.forEach(file => {
        const sourcePath = join(subdirPath, file);
        const targetPath = join(targetSubdirPath, file.replace('.tsx', '.ts').replace('.jsx', '.ts'));
...
${fleetManifest.commands.map(cmd => `
### ${cmd.title}
\`\`\`
cmd.${cmd.name}()
\`\`\`
