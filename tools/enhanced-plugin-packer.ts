...
class FleetChatPluginPacker {
  private converter: RaycastToLitConverter;
   ...
   */
  async package(pluginPath: string, outputPath: string): Promise<void> {
    try {
   ...
   */
  private async convertPluginFiles(pluginPath: string): Promise<Map<string, string>> {
  ...
  private createManifest(packageJson: any): PluginManifest {
    return {
      name: packageJson.name,
      version: packageJson.version,
      description: packageJson.description,
      author: packageJson.author,
      commands: packageJson.commands.map((cmd: any) => ({
        name: cmd.name,
        title: cmd.title,
...
program
  ...
  .action(async (pluginPath: string, outputPath: string, options) => {
    if (options.verbose) {
