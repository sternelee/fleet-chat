...
class PluginPackager {
  ...
  constructor(pluginDir: string, outFile: string) {
   ...
   */
  private extractManifest(packageJson: any): PluginManifest {
   ...
   * Create plugin metadata
   */
  private createMetadata(manifest: PluginManifest): PackageMetadata {
    return {
      manifest,
   ...
   */
  private writeManifestFiles(manifest: PluginManifest, metadata: PackageMetadata): void {
    // Write manifest.json
    writeFileSync(
    ...
    // Write metadata.json
    writeFileSync(
      join(this.tempDir, 'metadata.json'),
   ...
   * Create compressed package
   */
  private createCompressedPackage(): Promise<void> {
   ...
   */
  private recreatePackageWithChecksum(): Promise<void> {
    return new Promise((resolve, reject) => {
      const output = create('zip');
      const destination = require('fs').createWriteStream(this.outFile);
   ...
   */
  private addFilesToArchive(archive: any, dir: string, relativePath: string = ''): void {
   ...
   */
  private getFleetChatVersion(): string {
    try {
...
program
  ...
  .action(async (pluginDir: string, outputFile?: string, options?: any) => {
