...
class RaycastToLitConverter {
   ...
   */
  async convertComponent(filePath: string, options: ConversionOptions = {}): Promise<string> {
  ...
  private async parseAndConvert(content: string, options: ConversionOptions): Promise<string> {
  ...
  private updateImports(content: string, options: ConversionOptions): string {
    let updated = content;
    ...
    if (!options.preserveImports) {
      updated = updated.replace(
        /import\s+(React|{.*?React.*?})\s+from\s+['"]react['"];?\s*/g,
        ''
      );
    }
  ...
  private convertHooks(content: string): string {
    let converted = content;
  ...
  private convertJSX(content: string): string {
    let converted = content;
  ...
  private convertNestedJSX(content: string): string {
  ...
  private convertComponentProps(content: string): string {
    let converted = content;
  ...
  private convertEventHandlers(content: string): string {
    let converted = content;
  ...
  private convertPropsAndState(content: string): string {
    let converted = content;
...
`;
  ...
  private wrapAsLitClass(content: string, filePath: string): string {
...
`;
   ...
   */
  async convertDirectory(inputDir: string, outputDir: string, options: ConversionOptions = {}): Promise<void> {
...
export { RaycastToLitConverter, ConversionOptions };