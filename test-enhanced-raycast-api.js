    ...
    try {
      if (importName === '@raycast/api' || importName === '@raycast/utils') {
        ...
        try {
          ...
          results.passed++;
          results.details.push({ name: importName, success: true, note: 'Available after compilation' });
        }
      } else {
...
async function runEnhancedAPITests() {
  ...
  console.log(`Standard Import Compatibility: ${standardImportsTest.failed === 0 ? '✅ PASS' : '❌ FAIL'}`);

  const overallSuccess = enhancedTest.success &&
                        todoListTest.failed === 0 &&
                        standardImportsTest.failed === 0;

  console.log(`Overall Success: ${overallSuccess ? '🎉 FULLY COMPATIBLE' : '⚠️  PARTIALLY COMPATIBLE'}`);
...
// Run the tests
runEnhancedAPITests().then(result => {
